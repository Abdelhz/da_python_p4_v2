def sorting_list_score(list_of_players):
    list_players_sorted_score = sorted(list_of_players, key=lambda player: (player.score, player.ranking))
...
def create_new_pairs(list_players_sort, list_done_pairs):
    list_of_pairs = []
    list_player_tempo = list_players_sort
        ...
        if int(len(list_player_tempo)) > 2:
            ...
            for pair in list_done_pairs:
                if (pair_players[0].id_player == pair[0].id_player and pair_players[1].id_player == pair[1].id_player)\
                    ...
                    pair_exist = True
                    pair_players = [list_player_tempo[i], list_player_tempo[i+2]]
                    list_of_pairs.append(pair_players)
                    list_done_pairs.append(pair_players)
